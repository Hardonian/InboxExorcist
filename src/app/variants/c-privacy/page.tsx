...
export default function VariantCPrivacy() {
  return (
    <main className="min-h-screen bg-[#f8faf7] text-[#17150f]">
        ...
        <div className="mx-auto grid min-h-[92vh] max-w-7xl items-center gap-10 px-5 py-8 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:px-10">
          <div className="max-w-3xl">
            <p className="mb-5 inline-flex rounded-full border border-[#17150f]/15 bg-white px-3 py-1 text-sm font-medium">
              Privacy-first inbox management
            </p>
            <h1 className="text-5xl font-semibold leading-[1.02] tracking-normal sm:text-7xl">
              Quiet the noise. Keep your privacy.
            </h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#4d473b] sm:text-xl">
              InboxExorcist reads only email headers — never message bodies. Your data is encrypted, never sold, and fully deletable. Disconnect anytime with one click.
            </p>
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-lg border border-[#17150f]/15 bg-[#17150f] p-4 shadow-2xl">
              <div className="mb-4 flex items-center justify-between border-b border-white/10 pb-3 text-white">
                <span className="font-semibold">What we access</span>
              ...
              <div className="space-y-3">
                {[
                  ...
                  ["Hashed emails", "One-way, not reversible"],
                  ["Full deletion", "Request and it&apos;s gone"],
                ].map(([label, desc]) => (
                  <div
                    key={label}
                    className="rounded-md bg-white p-4"
                  >
                    <span className="text-sm font-semibold text-[#17150f]">{label}</span>
                    <span className="ml-2 text-sm text-[#4d473b]">{desc}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 rounded-md bg-[#d7ff7a] p-4 text-sm font-semibold text-[#17150f]">
                Your inbox data is yours. Always.
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-12 sm:px-8 lg:grid-cols-3 lg:px-10">
        {[
          { step: "01", title: "Minimal scopes", desc: "We request the narrowest Gmail access needed. Incremental auth means you grant more only when you choose to." },
          { step: "02", title: "Encrypted at rest", desc: "OAuth tokens and your Gmail email are AES-256-GCM encrypted. Sender emails are one-way hashed." },
          { step: "03", title: "Delete anytime", desc: "One click deletes all your data. OAuth tokens are revoked on disconnect. No retention games." },
        ].map((item) => (
